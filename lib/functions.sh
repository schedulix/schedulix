...
BICSUITEMKNOD=${BICSUITEMKNOD:-mknod}
...
export SDMSTIMEOUT BICSUITEGREP BICSUITEKILL BICSUITEFUSER BICSUITEMKNOD SCROLLLOG SDMSH SDMSUSER SDMSPASS SDMSHOST SDMSPORT
...
ensure_pipe()
{
	...
	if [ ! -p $LOGFILE ]; then
		...
		if $BICSUITEMKNOD $LOGFILE p; then
			: do nothing
		else
