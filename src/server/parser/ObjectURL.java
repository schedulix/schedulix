...
public class ObjectURL
{
		...
		switch (objType.intValue()) {
			...
			case SDMSObjectComment.SCOPE:
				...
				if (((SDMSScope) p).getType(sysEnv).intValue() == SDMSScope.SERVER)
					parserType = Integer.valueOf(Parser.JOB_SERVER);
				break;
		...
		throws SDMSException
	{
		...
		try {
			...
			objType = Integer.valueOf(SDMSObjectComment.RESOURCE_TEMPLATE);
		...
		throws SDMSException
	{
		...
		if (seSpec == null) {
			...
			final ObjectURL evtObj = new ObjectURL (Integer.valueOf(Parser.EVENT), evtName, seSpec);
			evtObj.resolveByName (sysEnv);
			evtId = evtObj.objId;

			final ObjectURL sceObj = new ObjectURL (Integer.valueOf(Parser.SCHEDULE), path, seSpec);
