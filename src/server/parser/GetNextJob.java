...
public class GetNextJob extends JobDistribution
{
		...
		throws SDMSException
	{
		...
		v = SDMSRunnableQueueTable.idx_scopeId_state.getVectorForUpdate(sysEnv, new SDMSKey(sId, Integer.valueOf(SDMSSubmittedEntity.STARTING)));
		...
		if (sme == null  && kj == null) {
			...
			v = SDMSRunnableQueueTable.idx_scopeId_state.getVectorForUpdate(sysEnv, new SDMSKey(sId, Integer.valueOf(SDMSSubmittedEntity.RUNNABLE)), filter);
			if(v.size() == 0) {
				if (sysEnv.maxWriter == 1) {
					sysEnv.tx.commitSubTransaction(sysEnv);
					SystemEnvironment.sched.getNextJobSchedule(sysEnv);
					sysEnv.tx.beginSubTransaction(sysEnv);
					v = SDMSRunnableQueueTable.idx_scopeId_state.getVector(sysEnv, new SDMSKey(sId, Integer.valueOf(SDMSSubmittedEntity.RUNNABLE)), filter);
				} else {
