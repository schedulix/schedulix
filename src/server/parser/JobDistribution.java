...
public abstract class JobDistribution extends Node
{
		...
		throws SDMSException
	{
		...
		sme.setState(sysEnv, Integer.valueOf(SDMSSubmittedEntity.STARTING));
		...
		for(int i = 0; i < jsv.size(); i++) {
			...
			if(rq.getScopeId(sysEnv).equals(sId)) {
				rq.setState(sysEnv, Integer.valueOf(SDMSSubmittedEntity.STARTING));
				continue;
			}
			rq.delete(sysEnv);
		}
		...
		throws SDMSException
	{
		...
		kj.setState(sysEnv, Integer.valueOf(SDMSSubmittedEntity.STARTING));
