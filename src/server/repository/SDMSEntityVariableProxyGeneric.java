...
public class SDMSEntityVariableProxyGeneric extends SDMSProxy
{
	...
	protected static SDMSEntityVariable getProxy (SystemEnvironment sysEnv, SDMSObject p_object)
	{
		int i = SDMSEntityVariableTable.table.tableIndex;
		SDMSProxy p = SDMSRepository.getProxy(i);
		if (p == null)
			p = new SDMSEntityVariable (p_object);
		else {
			p.initProxy(p_object);
		}
		sysEnv.tx.addUsedProxy(i, p);
		return (SDMSEntityVariable)p;
	}
	...
	{
		checkRead(env);
		return (((SDMSEntityVariableGeneric)(object)).getIsLong (env));
	}
	...
	throws SDMSException
	{
		...
		touchMaster(env);
		((SDMSEntityVariableGeneric)(object)).setIsLong (env, p_isLong);
		return ;
	}
