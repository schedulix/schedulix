...
public class SDMSResourceStateMappingProfileProxyGeneric extends SDMSProxy
{
	...
	protected static SDMSResourceStateMappingProfile getProxy (SystemEnvironment sysEnv, SDMSObject p_object)
	{
		int i = SDMSResourceStateMappingProfileTable.table.tableIndex;
		SDMSProxy p = SDMSRepository.getProxy(i);
		if (p == null)
			p = new SDMSResourceStateMappingProfile (p_object);
		else {
			p.initProxy(p_object);
		}
		sysEnv.tx.addUsedProxy(i, p);
		return (SDMSResourceStateMappingProfile)p;
	}
