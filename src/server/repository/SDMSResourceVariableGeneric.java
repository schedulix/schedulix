	...
	throws SDMSException
	{
		...
		if(pInsert[env.dbConnectionNr] == null) {
			try {
				...
				stmt =
				        "INSERT INTO " + squote + "RESOURCE_VARIABLE" + equote + " (" +
				        "ID" +
				        ", " + squote + "PD_ID" + equote +
				        ", " + squote + "R_ID" + equote +
				        ", " + squote + "VALUE" + equote +
		...
		try {
			...
			throw new SDMSSQLException(new SDMSMessage(env, "01110181954", "ResourceVariable: $1 $2", Integer.valueOf(sqle.getErrorCode()), sqle.getMessage()));
		}
	}
	...
	throws SDMSException
	{
		...
		if(pDelete[env.dbConnectionNr] == null) {
			try {
				...
				String equote = SystemEnvironment.EQUOTE;
				stmt =
				        "DELETE FROM " + squote + "RESOURCE_VARIABLE" + equote + " WHERE ID = ?";
		...
		try {
			...
			throw new SDMSSQLException(new SDMSMessage(env, "01110182002", "ResourceVariable: $1 $2", Integer.valueOf(sqle.getErrorCode()), sqle.getMessage()));
		}
	}
	...
	throws SDMSException
	{
		...
		if(pUpdate[env.dbConnectionNr] == null) {
			try {
				...
				String equote = SystemEnvironment.EQUOTE;
				stmt =
				        "UPDATE " + squote + "RESOURCE_VARIABLE" + equote + " SET " +
				        "" + squote + "PD_ID" + equote + " = ? " +
				        ", " + squote + "R_ID" + equote + " = ? " +
				        ", " + squote + "VALUE" + equote + " = ? " +
				        ", " + squote + "CREATOR_U_ID" + equote + " = ? " +
		...
		try {
			...
			throw new SDMSSQLException(new SDMSMessage(env, "01110182006", "ResourceVariable: $1 $2", Integer.valueOf(sqle.getErrorCode()), sqle.getMessage()));
		}
	}
