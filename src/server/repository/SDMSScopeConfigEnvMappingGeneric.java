	...
	implements Cloneable
{
	...
	throws SDMSException
	{
		...
		if (o.versions.o_v == null || o.versions.o_v.size() == 0 || o.subTxId != env.tx.subTxId) o = (SDMSScopeConfigEnvMappingGeneric) change(env);
		if (p_value != null && p_value.length() > 256) {
			throw new CommonErrorException (
			        new SDMSMessage(env, "01112141510",
			                        "(ScopeConfigEnvMapping) Length of $1 exceeds maximum length $2", "value", "256")
	...
	throws SDMSException
	{
		...
		if(pInsert[env.dbConnectionNr] == null) {
			try {
				...
				stmt =
				        "INSERT INTO " + squote + "SCOPE_CONFIG_ENVMAPPING" + equote + " (" +
				        "ID" +
				        ", " + squote + "KEY" + equote +
				        ", " + squote + "VALUE" + equote +
				        ", " + squote + "S_ID" + equote +
	...
	throws SDMSException
	{
		...
		if(pDelete[env.dbConnectionNr] == null) {
			try {
				...
				String equote = SystemEnvironment.EQUOTE;
				stmt =
				        "DELETE FROM " + squote + "SCOPE_CONFIG_ENVMAPPING" + equote + " WHERE ID = ?";
	...
	throws SDMSException
	{
		...
		if(pUpdate[env.dbConnectionNr] == null) {
			try {
				...
				String equote = SystemEnvironment.EQUOTE;
				stmt =
				        "UPDATE " + squote + "SCOPE_CONFIG_ENVMAPPING" + equote + " SET " +
				        "" + squote + "KEY" + equote + " = ? " +
				        ", " + squote + "VALUE" + equote + " = ? " +
				        ", " + squote + "S_ID" + equote + " = ? " +
				        ", " + squote + "CREATOR_U_ID" + equote + " = ? " +
