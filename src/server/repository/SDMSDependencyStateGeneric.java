	...
	implements Cloneable
{
	...
	throws SDMSException
	{
		...
		SDMSDependencyStateGeneric o = this;
		if (versions.id.longValue() < SystemEnvironment.SYSTEM_OBJECTS_BOUNDARY) {
			throw new CommonErrorException(
			        new SDMSMessage (env, "02112141636", "(DependencyState) Change of system object not allowed")
			);
		}
		if (o.versions.o_v == null || o.versions.o_v.size() == 0 || o.subTxId != env.tx.subTxId) o = (SDMSDependencyStateGeneric) change(env);
		if (p_condition != null && p_condition.length() > 1024) {
			throw new CommonErrorException (
	...
	throws SDMSException
	{
		...
		try {
			...
			throw new SDMSSQLException(new SDMSMessage(env, "01110181954", "DependencyState: $1 $2", Integer.valueOf(sqle.getErrorCode()), sqle.getMessage()));
		}
	}
	...
	throws SDMSException
	{
		...
		try {
			...
			throw new SDMSSQLException(new SDMSMessage(env, "01110181956", "DependencyState: $1 $2", Integer.valueOf(sqle.getErrorCode()), sqle.getMessage()));
		}
	}
