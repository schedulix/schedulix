	...
	implements Cloneable
{
	...
	throws SDMSException
	{
		...
		SDMSExitStateMappingProfileGeneric o = this;
		if (versions.id.longValue() < SystemEnvironment.SYSTEM_OBJECTS_BOUNDARY) {
			throw new CommonErrorException(
			        new SDMSMessage (env, "02112141636", "(ExitStateMappingProfile) Change of system object not allowed")
			);
		}
		if (o.versions.o_v == null || o.versions.o_v.size() == 0 || o.subTxId != env.tx.subTxId) o = (SDMSExitStateMappingProfileGeneric) change(env);
	...
	{
		return SDMSExitStateMappingProfile.getProxy(sysEnv, this);
	}
