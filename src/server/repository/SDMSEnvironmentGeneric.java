	...
	implements Cloneable
{
	...
	throws SDMSException
	{
		...
		if (o.versions.o_v == null || o.versions.o_v.size() == 0 || o.subTxId != env.tx.subTxId) o = (SDMSEnvironmentGeneric) change(env);
		if (p_condition != null && p_condition.length() > 1024) {
			throw new CommonErrorException (
	...
	throws SDMSException
	{
		...
		if(pInsert[env.dbConnectionNr] == null) {
			try {
				...
				stmt =
				        "INSERT INTO " + squote + "ENVIRONMENT" + equote + " (" +
				        "ID" +
				        ", " + squote + "NE_ID" + equote +
				        ", " + squote + "NR_ID" + equote +
		...
		try {
			...
			throw new SDMSSQLException(new SDMSMessage(env, "01110181954", "Environment: $1 $2", Integer.valueOf(sqle.getErrorCode()), sqle.getMessage()));
		}
	}
	...
	throws SDMSException
	{
		...
		if(pUpdate[env.dbConnectionNr] == null) {
			try {
				...
				String equote = SystemEnvironment.EQUOTE;
				stmt =
				        "UPDATE " + squote + "ENVIRONMENT" + equote +
				        " SET VALID_TO = ?, " +
				        "    CHANGE_TS = ?, " +
				        "    CHANGER_U_ID = ? " +
				        "WHERE ID = ?" +
		...
		try {
			...
			throw new SDMSSQLException(new SDMSMessage(env, "01110181956", "Environment: $1 $2", Integer.valueOf(sqle.getErrorCode()), sqle.getMessage()));
		}
	}
