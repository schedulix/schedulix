...
public class SDMSResourceProxyGeneric extends SDMSProxy
{
	...
	protected static SDMSResource getProxy (SystemEnvironment sysEnv, SDMSObject p_object)
	{
		int i = SDMSResourceTable.table.tableIndex;
		SDMSProxy p = SDMSRepository.getProxy(i);
		if (p == null)
			p = new SDMSResource (p_object);
		else {
			p.initProxy(p_object);
		}
		sysEnv.tx.addUsedProxy(i, p);
		return (SDMSResource)p;
	}
