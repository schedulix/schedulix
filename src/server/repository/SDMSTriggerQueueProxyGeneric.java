...
public class SDMSTriggerQueueProxyGeneric extends SDMSProxy
{
	...
	protected static SDMSTriggerQueue getProxy (SystemEnvironment sysEnv, SDMSObject p_object)
	{
		int i = SDMSTriggerQueueTable.table.tableIndex;
		SDMSProxy p = SDMSRepository.getProxy(i);
		if (p == null)
			p = new SDMSTriggerQueue (p_object);
		else {
			p.initProxy(p_object);
		}
		sysEnv.tx.addUsedProxy(i, p);
		return (SDMSTriggerQueue)p;
	}
