...
public class SDMSCalendarProxyGeneric extends SDMSProxy
{
	...
	protected static SDMSCalendar getProxy (SystemEnvironment sysEnv, SDMSObject p_object)
	{
		int i = SDMSCalendarTable.table.tableIndex;
		SDMSProxy p = SDMSRepository.getProxy(i);
		if (p == null)
			p = new SDMSCalendar (p_object);
		else {
			p.initProxy(p_object);
		}
		sysEnv.tx.addUsedProxy(i, p);
		return (SDMSCalendar)p;
	}
	...
	throws SDMSException
	{
		...
		s.add(getStarttime(sysEnv));

		gotIt = false;
		Long scevId = getScevId(sysEnv);
		if (!gotIt)
			try {
				s.add(SDMSScheduledEventTable.getObject(sysEnv, scevId).getSortKey(sysEnv));
				gotIt = true;
