...
public class SDMSnpJobFootprintProxyGeneric extends SDMSProxy
{
	...
	protected static SDMSnpJobFootprint getProxy (SystemEnvironment sysEnv, SDMSObject p_object)
	{
		int i = SDMSnpJobFootprintTable.table.tableIndex;
		SDMSProxy p = SDMSRepository.getProxy(i);
		if (p == null)
			p = new SDMSnpJobFootprint (p_object);
		else {
			p.initProxy(p_object);
		}
		sysEnv.tx.addUsedProxy(i, p);
		return (SDMSnpJobFootprint)p;
	}
