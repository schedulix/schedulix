	...
	implements Cloneable
{
	...
	throws SDMSException
	{
		...
		SDMSEventGeneric o = this;
		if (versions.id.longValue() < SystemEnvironment.SYSTEM_OBJECTS_BOUNDARY) {
			throw new CommonErrorException(
			        new SDMSMessage (env, "02112141636", "(Event) Change of system object not allowed")
			);
		}
		if (o.versions.o_v == null || o.versions.o_v.size() == 0 || o.subTxId != env.tx.subTxId) o = (SDMSEventGeneric) change(env);
	...
	throws SDMSException
	{
		...
		if(pInsert[env.dbConnectionNr] == null) {
			try {
				...
				stmt =
				        "INSERT INTO " + squote + "EVENT" + equote + " (" +
				        "ID" +
				        ", " + squote + "NAME" + equote +
		...
		try {
			...
			if (seId == null)
				...
				myInsert.setLong (4, seId.longValue());
			...
			throw new SDMSSQLException(new SDMSMessage(env, "01110181954", "Event: $1 $2", Integer.valueOf(sqle.getErrorCode()), sqle.getMessage()));
		}
	}
	...
	throws SDMSException
	{
		...
		if(pDelete[env.dbConnectionNr] == null) {
			try {
				...
				String equote = SystemEnvironment.EQUOTE;
				stmt =
				        "DELETE FROM " + squote + "EVENT" + equote + " WHERE ID = ?";
		...
		try {
			...
			throw new SDMSSQLException(new SDMSMessage(env, "01110182002", "Event: $1 $2", Integer.valueOf(sqle.getErrorCode()), sqle.getMessage()));
		}
	}
	...
	throws SDMSException
	{
		...
		if(pUpdate[env.dbConnectionNr] == null) {
			try {
				...
				String equote = SystemEnvironment.EQUOTE;
				stmt =
				        "UPDATE " + squote + "EVENT" + equote + " SET " +
				        "" + squote + "NAME" + equote + " = ? " +
				        ", " + squote + "OWNER_ID" + equote + " = ? " +
				        ", " + squote + "SE_ID" + equote + " = ? " +
				        ", " + squote + "CREATOR_U_ID" + equote + " = ? " +
		...
		try {
			...
			if (seId == null)
				...
				myUpdate.setLong (3, seId.longValue());
			...
			throw new SDMSSQLException(new SDMSMessage(env, "01110182006", "Event: $1 $2", Integer.valueOf(sqle.getErrorCode()), sqle.getMessage()));
		}
	}
