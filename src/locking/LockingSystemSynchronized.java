...
public class LockingSystemSynchronized
{
	...
	protected static synchronized Vector<ObjectLock> release()
	{
		...
		HashMap<Object, ObjectLock> locksHeld = threads.get(lt);
		if (locksHeld == null)
			return null;
	...
	protected static synchronized Vector<ObjectLock> release(Object object)
	{
		...
		ObjectLock lock = getLockForThread(lt, object);
		Vector<ObjectLock> locksToNotify = null;
		if (lock != null) {
			locksToNotify = releaseLock(lock);
			HashMap threadLocks = threads.get(lt);
	...
	throws DeadlockException
	{
		...
		if (lock != null) {
			...
			if (lock.mode != mode && lock.mode != ObjectLock.EXCLUSIVE) {
			...
			lock = ObjectLock.getObjectLock(lt, object, mode);
			registerLockForThread(lt, object, lock);
